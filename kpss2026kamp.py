    ...
    with t1:
        with st.form("login_form_unique"):
            ...
            if st.form_submit_button("Sisteme Bağlan", use_container_width=True):
                if u and p:
                    # Gereksiz str() dönüşümleri kaldırıldı
                    user_check = all_db[
    ...
    with t2:
        with st.form("reg_form_unique"):
            ...
            if st.form_submit_button("Hesap Oluştur", use_container_width=True):
                if nu in all_db['username'].values: 
                    st.error("Bu kullanıcı mevcut.")
                else:
                    new_u_row = pd.DataFrame([{
                        "username": nu, "display_name": nu, "password": hash_password(np), 
                        "tamamlandi": False, "id": int(time.time() * 1000), "konu": "Hesap Aktif", 
                        "soru_cozulen": 0, "soru_hedef": 1, "puan_hedef": 0.0, "ders": "Genel"
                    }])
                    save_to_gsheets(pd.concat([all_db, new_u_row], ignore_index=True))
                    st.success("Kayıt başarılı!")
    st.stop()

# --- 5. ANA EKRAN HAZIRLIKLARI ---
username = st.session_state.user
user_df = all_db[all_db['username'] == username].copy()

# Güvenli Hedef Puan Çekimi
if not user_df.empty and 'puan_hedef' in user_df.columns and pd.notna(user_df['puan_hedef'].iloc[0]):
    mevcut_hedef = float(user_df['puan_hedef'].iloc[0])
else:
    mevcut_hedef = 0.0
    
# Güvenli İsim Çekimi
if not user_df.empty and 'display_name' in user_df.columns and pd.notna(user_df['display_name'].iloc[0]):
...
with st.sidebar.expander("⚙️ Hesap Ayarları"):
    ...
    yeni_display = st.text_input("Ekranda Görünecek Adın", value=d_name)
    ...
    yeni_hedef = st.number_input("Hedef KPSS Puanı", min_value=0.0, max_value=100.0, value=mevcut_hedef, step=0.5)
    ...
    with st.expander("➕ Yeni Ders Ekle", expanded=False):
        ...
        with c_add3:
            if st.button("Ekle", use_container_width=True, type="primary"):
                if n_d: 
                    st.session_state.dersler[n_d] = st.session_state.selected_icon
                    st.rerun()
    ...
    with st.form("plan_form_ana", clear_on_submit=True):
        ...
        if submit_btn:
            if k_a:
                ...
                n_p = pd.DataFrame([{
                    "username": username, "password": user_df['password'].iloc[0], # Güvenli şifre çekimi
            ...
            with st.expander(f"{ikon} {row['ders']} - {row['konu']}", expanded=True):
                ...
                with cr:
                    ...
                    if st.button("🗑️ Planı Sil", key=f"del_act_{row['id']}", use_container_width=True):
                        save_to_gsheets(all_db[all_db['id'] != row['id']])
                        st.toast(f"{row['konu']} konusunu sildiniz!", icon="🗑️")
                        time.sleep(0.3)
                        st.rerun()
...
# --- 8. BAŞARILARIM ---
    ...
    for d, ikon in st.session_state.dersler.items():
        ...
        if not ders_df.empty:
            ...
            toplam_video = sum([len(json.loads(r['videolar'])) for _, r in b_df.iterrows() if isinstance(r['videolar'], str)])
...
# --- 9. DENEME TAKİBİ ---
    ...
    hedef_puan = mevcut_hedef if mevcut_hedef > 0 else 75.0
        ...
        if st.button("🚀 Denemeyi Arşive Kaydet", use_container_width=True):
            if d_ad:
                ...
                yeni_deneme = pd.DataFrame([{
                    ...
                    "deneme_gk_d": gk_d, "deneme_gk_y": gk_y, "deneme_gy_d": gy_d, "deneme_gy_y": gy_y,
                    "deneme_puan": float(hesaplanan_puan), "puan_hedef": float(hedef_puan),
                    "tamamlandi": True, "id": int(time.time() * 1000), "videolar": "[]",
    ...
    if deneme_gecmisi.empty:
        ...
        for _, d_row in deneme_gecmisi.iterrows():
            ...
            if fark >= 0:
                # Başarı mesajı döngü içinden çıkarıldı, sadece değişkene aktarıldı
