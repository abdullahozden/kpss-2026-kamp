    ...
    if active_df.empty: st.info("Aktif görev yok.")
        ...
        for date_val in unique_dates:
            try:
                ...
                display_date = f"{date_obj.day} {aylar_tr[date_obj.month]} {date_obj.year} <span style='font-size: 0.8em; color: #8a9ba8; font-weight: normal; margin-left: 6px;'>{gunler_tr[date_obj.weekday()]}</span>"
            except:
            ...
            for idx, row in day_tasks.iterrows():
                ...
                with st.expander(f"{ikon} {row['ders']} - {row['konu']}", expanded=False):
                    ...
                    with cr:
                        ...
                        col_m2.metric("Çözülen", c_q)
                        n_q = st.number_input("Sayıyı Güncelle", value=c_q, key=f"q_{row['id']}", label_visibility="collapsed", step=1)
                        if n_q != c_q:
